...
function MyFoldText() 
  let fsec = substitute(getline(v:foldstart),'{.*','{','')
  let fsec = substitute(fsec, ' ', '-', 'g')
  let fsec = substitute(fsec, '\t', '----', 'g')
  let fsec = substitute(fsec, '-', '+', '')

  let nlines = v:foldend - v:foldstart + 1 

  let sub = ''
  let charline = GetFirstLineWithChars()
  if charline < nlines
    let sub = getline(v:foldstart + charline)
    let sub = substitute(sub, '[\t ]*', '', '')
  endif

  return fsec . " " . nlines . " } " . sub . " "
endfunction
